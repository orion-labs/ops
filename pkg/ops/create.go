...
func (s *Stack) KotsInstall(sshClient *SshProgClient) (err error) {
	...
	cmd := fmt.Sprintf("sudo -i kubectl kots install orion-ptt-system --license-file /home/%s/license.yaml --shared-password %s --namespace default --config-values /home/%s/config.yaml", s.Config.Username, s.Config.KotsadmPassword, s.Config.Username)
